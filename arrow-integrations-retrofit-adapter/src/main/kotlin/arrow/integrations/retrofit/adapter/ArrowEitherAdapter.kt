...
internal class ArrowEitherAdapter<E, R>(
    retrofit: Retrofit,
    errorType: Type,
        ...
        override fun enqueue(callback: Callback<Either<E, R>>) {
            original.enqueue(object : Callback<R> {
                ...
                override fun onResponse(call: Call<R>, response: Response<R>) {
                  val er: Response<Either<E, R>> = if (response.isSuccessful) {
                        val body = response.body()!!.right()
                        Response.success(response.code(), body)
                    } else {
                        val error = errorConverter.convert(response.errorBody()!!)!!.left()
                        Response.success(error)
                    }
                    callback.onResponse(this@EitherCall, er)
                }
            })
        }
