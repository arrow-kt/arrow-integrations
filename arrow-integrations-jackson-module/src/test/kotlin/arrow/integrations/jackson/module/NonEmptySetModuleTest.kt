...
class NonEmptySetModuleTest {
  ...
  @Test fun `serializing NonEmptySet should be the same as serializing the underlying set`() = runTest {
    checkAll(
  ...
  @Test fun `serializing NonEmptySet and then deserialize it should be the same as before the deserialization`() = runTest {
    checkAll(
      Arb.choice(
        arbitrary {
        ...
        arbitrary { Arb.nonEmptySet(Arb.string()).bind() to jacksonTypeRef<NonEmptySet<String>>() },
        arbitrary { Arb.nonEmptySet(Arb.boolean()).bind() to jacksonTypeRef<NonEmptySet<Boolean>>() }
      )
    ) { (set, typeReference) ->
  ...
  @Test fun `serializing NonEmptySet in an object should round trip`() = runTest {
  ...
  @Test fun `should round trip on NonEmptySet with wildcard type`() = runTest {
