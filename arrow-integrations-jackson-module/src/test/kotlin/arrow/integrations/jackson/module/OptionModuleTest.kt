...
class OptionModuleTest {
  ...
  @Test fun `serializing Option should be the same as serializing a nullable value`() = runTest {
  ...
  @Test fun `serializing Option with NON_ABSENT should honor such configuration and omit serialization when option is empty`() = runTest {
  ...
  @Test fun `serializing Option and then deserialize it should be the same as before the deserialization`() = runTest {
    checkAll(
      Arb.choice(
